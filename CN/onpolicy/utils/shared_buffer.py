...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        ...
        if args.use_dbc:
            share_obs_dim = share_obs_shape[0] + self.args.dbc_out_dim*self.args.num_agents
            obs_dim = obs_shape[0] + self.args.dbc_out_dim
            self.share_obs = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents, share_obs_dim), dtype=np.float32)
        ...
        if args.use_graph:
            self.intentions = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents, args.embed_size), dtype=np.float32)
            self.groups = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents, args.embed_size), dtype=np.float32)
        else:
            self.intentions, self.groups = None, None
            
            
    def insert(self, share_obs, obs, rnn_states_actor, rnn_states_critic, actions, action_log_probs,
               value_preds, rewards, masks, bad_masks=None, active_masks=None, available_actions=None, intentions=None, groups=None):
        """
        ...
        if intentions is not None:
            self.intentions[self.step + 1] = intentions.copy()
            self.groups[self.step + 1] = groups.copy()
    ...
    def after_update(self):
        ...
        if self.intentions is not None:
            ...
            self.groups[0] = self.groups[-1].copy()
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        next_share_obs = self.share_obs[1:].reshape(-1, *self.share_obs.shape[3:])
        ...
        self.joint_act = np.repeat(self.actions[:,:,:,0][:,:,None], num_agents, 2)
        joint_act = self.joint_act.reshape(-1, self.joint_act.shape[-1])
        ...
        if self.intentions is not None:
            intentions = self.intentions[:-1].reshape(-1, *self.intentions.shape[3:])
            groups = self.groups[:-1].reshape(-1, *self.groups.shape[3:])
        ...
        for indices in sampler:
            ...
            joint_act_batch = joint_act[indices]
            ...
            if self.intentions is not None:
                intentions_batch = intentions[indices]
                groups_batch = groups[indices]
            else:
                intentions_batch, groups_batch = None, None
            ...
            if self.intentions is not None:
                yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch,\
                    ...
                    adv_targ, available_actions_batch, intentions_batch, groups_batch, next_obs_batch, next_share_obs_batch, joint_act_batch
            else:
                yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch,\
                    value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch,\
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        if len(self.share_obs.shape) > 4:
            ...
            next_share_obs = _cast(self.share_obs[1:])
            
            self.joint_act = np.repeat(self.actions[:,:,:,0][:,:,None], num_agents, 2)

        actions = _cast(self.actions)
        joint_act = _cast(self.joint_act)
        ...
        if self.intentions is not None:
            intentions = _cast(self.intentions[:-1])
            groups = _cast(self.groups[:-1])
        ...
        for indices in sampler:
            ...
            for index in indices:
                ...
                joint_act_batch.append(joint_act[ind:ind + data_chunk_length])
                
                if self.available_actions is not None:
                    available_actions_batch.append(available_actions[ind:ind + data_chunk_length])
                if self.intentions is not None:
                    intention_batch.append(intentions[ind:ind + data_chunk_length])
                    group_batch.append(groups[ind:ind + data_chunk_length])
            ...
            joint_act_batch = np.stack(joint_act_batch, axis=1)
            
            if self.available_actions is not None:
                available_actions_batch = np.stack(available_actions_batch, axis=1)
            if self.intentions is not None:
                intention_batch = np.stack(intention_batch, axis=1)
                group_batch = np.stack(group_batch, axis=1)
            ...
            next_share_obs_batch = _flatten(L, N, next_share_obs_batch)
            obs_batch = _flatten(L, N, obs_batch)
            next_obs_batch = _flatten(L, N, next_obs_batch)
            actions_batch = _flatten(L, N, actions_batch)
            joint_act_batch = _flatten(L, N, joint_act_batch)
            if self.available_actions is not None:
            ...
            if self.intentions is not None:
                intention_batch = _flatten(L, N, intention_batch)
                group_batch = _flatten(L, N, group_batch)
            ...
            if self.intentions is not None:
                yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch,\
                    value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch,\
                    adv_targ, available_actions_batch, intention_batch, group_batch, next_obs_batch, next_share_obs_batch, joint_act_batch
            else:
                yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch,\
                    value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch,\
                    adv_targ, available_actions_batch
