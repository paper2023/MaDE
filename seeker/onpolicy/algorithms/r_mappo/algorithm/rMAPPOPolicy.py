...
class R_MAPPO_GRAPH_Policy:
    ...
    def __init__(self, args, obs_space, cent_obs_space, act_space, device=torch.device("cpu")):
        ...
        hidden_size, output_size, num_actions = 128, self.args.embed_size, 5
            
        if self.args.env_name == 'mpe':
            from onpolicy.algorithms.bis_dbc.dbc_agent import BisimAgent
            self.bis_agent_global = BisimAgent(self.global_obs_size, hidden_size, output_size, self.joint_action_shape, transition_model_type) 
            self.bis_agent_local = BisimAgent(self.obs_size, hidden_size, output_size, num_actions, transition_model_type) 
        elif self.args.env_name == 'seeker':
            from onpolicy.algorithms.bis_dbc.dbc_agent_seeker import BisimAgent, BisimAgent_GLOBAL
            self.bis_agent_global = BisimAgent_GLOBAL(self.global_obs_size, hidden_size, output_size, self.joint_action_shape, transition_model_type) 
            self.bis_agent_local = BisimAgent(self.obs_size, hidden_size, output_size, num_actions, transition_model_type) 
        ...
        if self.args.use_graph:
            ...
            if self.args.env_name == 'mpe':
                ...
                # if self.args.global_bis_agents_path != '':
                self.bis_agent_global.load_state_dict(torch.load(self.args.global_bis_agents_path))
                self.bis_agent_local.load_state_dict(torch.load(self.args.local_bis_agents_path))
    ...
    def get_actions(self, cent_obs, obs, rnn_states_actor, rnn_states_critic, masks, final_obs, cur_group_lst, available_actions=None,
        ...
        if self.args.abl_flag == '_del_graph': # # abl_flag=0 删除 态势图   # abl_flag=1 删除组别  # abl_flag=2 删除obs
            cent_obs_cat = np.concatenate((cent_obs, group_tensors_), axis=-1) # torch.Size([192, 150]) # # torch.Size([192, 64]) #
        elif self.args.abl_flag == '_del_group':
            cent_obs_cat = np.concatenate((cent_obs, intention_tensors_), axis=-1)  # torch.Size([192, 150])  # torch.Size([192, 64]) #
        elif self.args.abl_flag == '_del_obs':
            cent_obs_cat = np.concatenate((intention_tensors_, group_tensors_), axis=-1)  # torch.Size([192, 64]) # # torch.Size([192, 64]) #
...
class R_MAPPOPolicy:
    ...
    def __init__(self, args, obs_space, cent_obs_space, act_space, device=torch.device("cpu")):
        ...
        if self.args.use_dbc:
            from onpolicy.algorithms.bis_dbc.dbc_agent import BisimAgent
            hidden_size, output_size, action_shape, transition_model_type = 128, 64, 5, 'ensemble'  
            ...
            self.bis_model.load_state_dict(torch.load(self.args.local_bis_agents_path))
