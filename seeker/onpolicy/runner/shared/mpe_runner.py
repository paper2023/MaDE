        ...
        for episode in range(episodes):
            ...
            for step in range(self.episode_length):
                ...
                if step == 0: ### design the goal
                    ### get_final_goal
                    final_obs = np.repeat(self.envs.design_final_goal(), self.all_args.num_agents, axis=1)
                    # final_obs = np.array([self.policy.cluster.design_final_goal_seeker(i) for i in self.buffer.obs[step]])         
                    obs_ = check(torch.from_numpy(self.buffer.obs[step])).to(**self.tpdv)      
                    obs_1 = obs_.reshape(-1, obs_.shape[-1])  
                    local_h = self.trainer.policy.bis_agent_local.encoder(obs_1)
                    local_h_ = local_h.reshape(*obs_.shape[:2], -1)
                    self.cur_group_lst = [self.trainer.policy.cluster.construct_groups(i) for i in local_h_]
            ...
            if episode % self.log_interval == 0:
                ...
                # if self.env_name == "MPE"  or self.env_name == "seeker":
                #     env_infos = {}
                ...
                if self.env_name == "seeker":
                    env_infos = {}
                    env_infos['agent0/individual_rewards'] = self.buffer.rewards[:,:,0]
    ...
    def test(self):
        ...
        for eps_ in range(1, N):
            ...
            if eps_ % slice_ == 0:
                ...
                np.save(self.all_args.test_save_path + 'o_a_r_o_prime_done_s_s_prime_u-slices_' + str(slices) + '-num_' + str(num) + '.npy', data_all, allow_pickle=True)
                eval_obs_lsts, eval_act_lsts, eval_rew_lsts, eval_done_lsts, eval_next_obs_lsts = [], [], [], [], []
        ...
        np.save(self.all_args.test_save_path + 'o_a_r_o_prime_done_s_s_prime_u' + '-num_' + str(num)  + '_done.npy', data_all_done, allow_pickle=True)
        # eval_episode_rewards = np.array(eval_episode_rewards)
        # eval_env_infos = {}
        # eval_env_infos['eval_average_episode_rewards'] = np.sum(np.array(eval_episode_rewards), axis=0)
        # eval_average_episode_rewards = np.mean(eval_env_infos['eval_average_episode_rewards'])
...
class MPERunner(Runner):
    ...
    def insert(self, data):
        ...
        if self.all_args.env_name == 'seeker':
            avail_actions = np.stack([i['avail_actions'] for i in infos])
        else:
            avail_actions = None

        self.buffer.insert(share_obs, obs, rnn_states, rnn_states_critic, actions, action_log_probs, values, rewards, masks, available_actions=avail_actions)
    ...
    def test(self):
        ...
        for eps_ in range(1, N):
            eval_obs_lst, eval_act_lst, eval_rew_lst, eval_done_lst, eval_next_obs_lst = [], [], [], [], []
            eval_obs, eval_ava = self.eval_envs.reset()
            ...
            for eval_step in range(self.episode_length):
                ...
                eval_action, eval_rnn_states = self.trainer.policy.act(np.concatenate(eval_obs),
                                                    np.concatenate(eval_rnn_states),
                                                    np.concatenate(eval_masks),
                                                    available_actions=np.concatenate(eval_ava),
                                                    deterministic=True)
                eval_actions = np.array(np.split(_t2n(eval_action), self.n_eval_rollout_threads))
                eval_rnn_states = np.array(np.split(_t2n(eval_rnn_states), self.n_eval_rollout_threads))
            ...
            if np.sum(np.vstack(eval_done_lst)) > 0 :
                dones_flag += 1
                eval_obs_lsts_done.append(np.vstack(eval_obs_lst))
                eval_act_lsts_done.append(np.vstack(eval_act_lst))
                eval_rew_lsts_done.append(np.vstack(eval_rew_lst))
            ...
            if eps_ % slice_ == 0:
                ...
                if not self.algorithm_name == 'mappo':
                    global_s = np.array(eval_obs_lsts).reshape(*(np.array(eval_obs_lsts).shape[:2]), -1)[:,:,None].repeat(self.num_agents ,axis=2)
                    global_next_s = np.array(eval_next_obs_lsts).reshape(*(np.array(eval_next_obs_lsts).shape[:2]), -1)[:,:,None].repeat(self.num_agents ,axis=2)
                    global_a = np.array(eval_act_lsts).reshape(*(np.array(eval_act_lsts).shape[:2]), -1)[:,:,None].repeat(self.num_agents ,axis=2)
                    ...
                    print('------------------------------save for not global----------------------------')
                    data_all = np.concatenate((np.array(eval_obs_lsts), np.array(eval_act_lsts), np.array(eval_rew_lsts), np.array(eval_next_obs_lsts), np.array(eval_done_lsts)), axis=-1)
                    num = data_all.shape[0]
                    np.save(self.all_args.test_save_path + 'o_a_r_o_prime_done-slices_' + str(slices) + '-num_' + str(num) + '.npy', data_all, allow_pickle=True)
                    eval_obs_lsts, eval_act_lsts, eval_rew_lsts, eval_done_lsts, eval_next_obs_lsts = [], [], [], [], []
                    print("eval dones number: " + str(dones_flag))
        ...
        if not self.algorithm_name == 'mappo':
            ...
            data_all_done = np.concatenate((np.array(eval_obs_lsts_done), np.array(eval_act_lsts_done), np.array(eval_rew_lsts_done), np.array(eval_next_obs_lsts_done), np.array(eval_done_lsts_done)), axis=-1)
            num = data_all_done.shape[0]
            np.save(self.all_args.test_save_path + 'o_a_r_o_prime_done' + '-num_' + str(num)  + '_done.npy', data_all_done, allow_pickle=True)
        # eval_episode_rewards = np.array(eval_episode_rewards)
        # eval_env_infos = {}
        # eval_env_infos['eval_average_episode_rewards'] = np.sum(np.array(eval_episode_rewards), axis=0)
        # eval_average_episode_rewards = np.mean(eval_env_infos['eval_average_episode_rewards'])
    ...
    def test_seeker(self):
        ...
        for eps_ in range(1, N):
            ...
            for eval_step in range(self.episode_length):
                ...
                eval_action, eval_rnn_states = self.trainer.policy.act(np.concatenate(eval_obs),
                                                    ...
                                                    available_actions=np.concatenate(eval_ava),
                                                    deterministic=True)
                eval_actions = np.array(np.split(_t2n(eval_action), self.n_eval_rollout_threads))  # (4, 12, 1)
                
                # if np.unique(eval_actions)[0] == 1:
                # print(np.unique(eval_actions))
                ...
                eval_obs_lst.append(eval_obs)
                eval_obs, eval_rewards, eval_dones, eval_infos = self.eval_envs.step(eval_actions_env)
                eval_act_lst.append(eval_actions) # (4, 12, 1)
                eval_rew_lst.append(eval_rewards)
                eval_done_lst.append(eval_dones[:,:,None])
                eval_next_obs_lst.append(eval_obs)
            ...
            if eps_ % slice_ == 0:
                ...
                if not self.algorithm_name == 'mappo':
                    ...
                    data_all = np.concatenate((np.array(eval_obs_lsts), np.array(eval_act_lsts), np.array(eval_rew_lsts), np.array(eval_next_obs_lsts), np.array(eval_done_lsts), \
                        global_s, global_next_s, global_a), axis=-1)
                    num = data_all.shape[0]
                    np.save(self.all_args.test_save_path + 'o_a_r_o_prime_done_s_s_prime_u-slices_' + str(slices) + '-num_' + str(num) + '_random.npy', data_all, allow_pickle=True)
                    eval_obs_lsts, eval_act_lsts, eval_rew_lsts, eval_done_lsts, eval_next_obs_lsts = [], [], [], [], []
                    print("eval dones number: " + str(dones_flag))
                else:
                    print('------------------------------save for not global----------------------------')
                    data_all = np.concatenate((np.array(eval_obs_lsts), np.array(eval_act_lsts), np.array(eval_rew_lsts), np.array(eval_next_obs_lsts), np.array(eval_done_lsts)), axis=-1)
                    num = data_all.shape[0]
                    np.save(self.all_args.test_save_path + 'o_a_r_o_prime_done-slices_' + str(slices) + '-num_' + str(num) + '_random.npy', data_all, allow_pickle=True)
                    eval_obs_lsts, eval_act_lsts, eval_rew_lsts, eval_done_lsts, eval_next_obs_lsts = [], [], [], [], []
                    print("eval dones number: " + str(dones_flag))
        ...
        if not self.algorithm_name == 'mappo':
            data_all_done = np.concatenate((np.array(eval_obs_lsts_done), np.array(eval_act_lsts_done), np.array(eval_rew_lsts_done), np.array(eval_next_obs_lsts_done), np.array(eval_done_lsts_done),\
                np.array(eval_global_s), np.array(eval_global_next_s), np.array(eval_global_a)), axis=-1)
            num = data_all_done.shape[0]
            np.save(self.all_args.test_save_path + 'o_a_r_o_prime_done_s_s_prime_u' + '-num_' + str(num)  + '_done_random.npy', data_all_done, allow_pickle=True)
        else:
            data_all_done = np.concatenate((np.array(eval_obs_lsts_done), np.array(eval_act_lsts_done), np.array(eval_rew_lsts_done), np.array(eval_next_obs_lsts_done), np.array(eval_done_lsts_done)), axis=-1)
            num = data_all_done.shape[0]
            np.save(self.all_args.test_save_path + 'o_a_r_o_prime_done' + '-num_' + str(num)  + '_done_random.npy', data_all_done, allow_pickle=True)
        # eval_episode_rewards = np.array(eval_episode_rewards)
