...
def worker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        if cmd == 'step':
            ...
            ob = env.reset()
            remote.send((ob))
        elif cmd == 'design_final_goal':
            f_g = env.design_final_goal()
            remote.send((f_g))
        elif cmd == 'render':
            if data == "rgb_array":
...
class SubprocVecEnv(ShareVecEnv):
    def __init__(self, env_fns, spaces=None, all_args=None):
        """
        envs: list of gym environments to run in subprocesses
        """
    ...
    def step_wait(self):
        ...
        self.waiting = False
        obs, rews, dones, infos = zip(*results)
        if type(obs[0]).__name__ == 'tuple':
            obs_, ava = zip(*obs)
            return np.stack(obs_), np.stack(rews), np.stack(dones), infos
        else:
            for i in range(len(obs)):
                if type(obs[i]).__name__ == 'tuple':
                    obs_, ava = obs[i] ############ 此处相当于环境被重置了
                    obs[i] = obs_
                    infos[i]['avail_actions'] = ava  
            return np.stack(obs), np.stack(rews), np.stack(dones), infos
    ...
    def reset(self):
        ...
        if self.all_args.env_name == 'seeker':
            return np.stack([i[0] for i in obs]), np.stack([i[1] for i in obs])
        else:
            return obs
    ...
    def design_final_goal(self):
        for remote in self.remotes:
            remote.send(('design_final_goal', None))
        obs = [remote.recv() for remote in self.remotes]
        # return np.stack(obs)
        if self.all_args.env_name == 'seeker':
            return np.stack([i for i in obs])
