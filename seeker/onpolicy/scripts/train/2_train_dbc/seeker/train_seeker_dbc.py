...
if __name__ == "__main__":
    for i in range(0, 5):
        ...
        args = ['--env_name', 'seeker', '--algorithm_name', 'rmappo', '--experiment_name', '4agents', '--num_agents', '4', '--num_landmarks', '4', '--scenario_name', 'simple_spread', 
        '--seed', '1', '--n_training_threads', '8', '--n_rollout_threads', '64', '--num_mini_batch', '16', '--episode_length', '50', '--num_env_steps', '6000000', 
        '--ppo_epoch', '10', '--gain', '0.01', '--lr', '7e-4', '--critic_lr', '7e-4', '--use_wandb', 'False']
        # 
        ...
        agents_lst = ['3', '5', '8', '12']
        model_path_root = '/data/jqruan/clustering/on-policy-seeker-161-20230320/onpolicy/scripts/train/2_train_dbc/seeker/bis_dbc_model/'
        # model_path =
        model_num = [
        ...
        experiment_name = num_agents + 'agents_' + args[3] + '_dbc' + '_num_{}'.format(model_num[index][num_index]) + '_mini16'
        
        args[5], args[7], args[9] = experiment_name, num_agents, num_agents
        
        args.extend(['--use_test', 'False', '--use_graph', 'False', '--use_ReLU', '--use_dbc'])

        args.extend(['--local_bis_agents_path', model_path_root + agents_lst[index] + 'agents/bis_agent._{}.pt'.format(model_num[index][num_index])])
        
        
        args[13]=str(i)### seed
        main(args)

        ### cd /data/jqruan/clustering/on-policy-seeker-161-20230320/onpolicy/scripts/train/2_train_dbc/seeker; conda activate pyg
        ### CUDA_VISIBLE_DEVICES=2 python train_seeker_dbc.py
