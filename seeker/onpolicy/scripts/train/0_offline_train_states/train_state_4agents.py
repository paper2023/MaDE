...
setproctitle.setproctitle("bq-abstract2")
...
#root_path = '/data2/xingdp/jqruan/yuzhongwei/clustering/on-policy-mpe/onpolicy/scripts/results/MPE/simple_spread/rmappo/3agents/run1/models/'
root_path = '/home/jqruan/data/clustering/on-policy-mpe/onpolicy/scripts/results/MPE/simple_spread/rmappo/4agents/run1/models/'
path_files = [
...
num_agents = 4
action_shape = 5
input_size, hidden_size, output_size = 24, 64, 16
...
log_dir = root_path + 'train_state_logs/' + transition_model_type + '_use_MI_'  if USE_MI else \
    root_path + 'train_state_logs/' + transition_model_type + '_' 
...
bis_agent_global = BisimAgent(global_size, hidden_size, output_size, joint_action_shape, transition_model_type)
...
for i in path_files:
    ...
    for _ in range(500):
        for ind, data in enumerate(tqdm(datas)):
            ...
            obs = data[:, :input_size].float() # obs
            act = data[:, [input_size]].long() # act
            rew = data[:, [input_size+1]].float() # rew
            next_obs = data[:, input_size+1:input_size+1+input_size].float() # next_obs
            
            global_s = data[:, -global_size-global_size-num_agents:-global_size-num_agents].float() # global_s
            next_global_s = data[:, -global_size-num_agents:-num_agents].float() # next_global_s
            joint_act = data[:, -num_agents:].long()
            
            action = torch.zeros(batch_size, action_shape).scatter_(1, act, 1) ## transform to one hot 
            ...
            if USE_MI:
                ...
                inds = torch.arange(bs*ne).unsqueeze(0).repeat(t,1) # torch.Size([1, 640])
